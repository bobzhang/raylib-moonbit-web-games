...
// include: /tmp/force_webgl1.pre.js
if (typeof Module !== 'object') Module = {};
const __oldPreRun = Module.preRun;
Module.preRun = Module.preRun || [];
if (typeof __oldPreRun === 'function') Module.preRun = [__oldPreRun, ...Module.preRun];
else if (Array.isArray(__oldPreRun)) Module.preRun = [...__oldPreRun, ...Module.preRun];

Module.preRun.push(function() {
  if (typeof Browser !== 'object' || typeof Browser.createContext !== 'function') return;
  const createContextOriginal = Browser.createContext;
  Browser.createContext = function(canvas, useWebGL, setInModule, webGLContextAttributes) {
    if (useWebGL) {
      const attrs = webGLContextAttributes ? { ...webGLContextAttributes } : {};
      // Force WebGL1 for better compatibility with raylib's HTML5 path.
      attrs.majorVersion = 1;
      return createContextOriginal.call(Browser, canvas, useWebGL, setInModule, attrs);
    }
    return createContextOriginal.call(Browser, canvas, useWebGL, setInModule, webGLContextAttributes);
  };
});
// end include: /tmp/force_webgl1.pre.js
