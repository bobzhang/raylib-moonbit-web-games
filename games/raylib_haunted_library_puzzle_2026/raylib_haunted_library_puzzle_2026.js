...
var ASM_CONSTS = {
 ...
 107898: () => { return parseInt(document.getElementById('canvas').style.width); },  
 107966: () => { document.exitFullscreen(); },  
 107993: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 108066: () => { if (document.fullscreenElement) return 1; },  
 108112: () => { return document.getElementById('canvas').width; },  
 108164: () => { return screen.width; },  
 108189: () => { document.exitFullscreen(); },  
 108216: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 108349: () => { return window.innerWidth; },  
 108375: () => { return window.innerHeight; },  
 108402: () => { if (document.fullscreenElement) return 1; },  
 108448: () => { return document.getElementById('canvas').width; },  
 108500: () => { return parseInt(document.getElementById('canvas').style.width); },  
 ...
 108691: () => { return window.innerWidth; },  
 108717: () => { return window.innerHeight; },  
 ...
 108867: () => { document.exitFullscreen(); },  
 108894: () => { if (document.fullscreenElement) return 1; },  
 108940: () => { return document.getElementById('canvas').width; },  
 108992: () => { return parseInt(document.getElementById('canvas').style.width); },  
 109060: () => { document.exitFullscreen(); },  
 109087: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 109145: () => { return screen.width; },  
 109170: () => { return screen.height; },  
 109196: () => { return window.screenX; },  
 109223: () => { return window.screenY; },  
 109250: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 109303: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 109374: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 109431: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 109687: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 109758: () => { if (document.fullscreenElement) return 1; },  
 109804: () => { return window.innerWidth; },  
 109830: () => { return window.innerHeight; },  
 109857: () => { if (document.pointerLockElement) return 1; },  
 109904: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 112062: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 112226: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 112330: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 115329: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 115395: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 115788: ($0) => { miniaudio.untrack_device_by_index($0); },  
 115831: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 115956: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
